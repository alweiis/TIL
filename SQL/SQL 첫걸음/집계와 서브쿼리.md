...
- 스칼라 값
  ...
  이때 위의 패턴에서 1번과 다른 패턴과 다르다. 이는 다른 패턴과 달리 하나의 값을 반홚하기 때문이다.
...
1. EXISTS
   ...
   - 서브쿼리를 사용해 검색할 때 EXISTS 술어를 사용하여 '데이터가 존재하는지 아는지' 판별하는 조건을 지정할 수 있다.
...
3. 상관 서브쿼리
   ...
   - 서브쿼리의 WHERE 구의 조건이 같은 이름의 열을 가르킨다면 어떻게 될까? 예를들어 'WHERE no = no'와 같이 지정한다면 제대로 동작할까?
...
4. IN
   ...
   - IN에서 오른쪽에 집합을 지정한다. 왼쪽에 지정된 값과 같은 값이 집합 안에 존재하면 참을 반환한다. 집합은 상수 리스트를 괄호로 묶어 기술한다.
   ...
   - 이 같은 경우 서브쿼리는 스칼라 서브쿼리가 될 필요는 없다. IN에는 집합을 지정할 수 있기 때문에 여러 행으로 이루어진 서브쿼리도 가능하다.
